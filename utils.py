...
def plot_graph(dloss, gloss):
    num_terms = len(dloss)
    for i in range(num_terms):
        print(dloss[i], "  ", gloss[i])

    x = range(num_terms)
    fig1 = plt.figure()
    plt.plot(x, dloss, 'r', label="dis_loss")
    plt.plot(x, gloss, 'b', label="gen_loss")
    plt.xlabel("iterations")
    plt.ylabel("losses")
    plt.title("Losses for "+str(num_terms)+" iterations")
    plt.legend()
...
def get_frames(file, nof):
    ...
    for frame in vid:
        ...
        resized_frame = skimage.transform.resize(frame, [64, 64])
        curr_frames.append(resized_frame)
        i = i + 1
        if i == nof:
            i = 0
            video.append(curr_frames)
...
def load_data(files, nof):
    videos = []
    total_frames = 0
    for file in files:
        dummy_vid, num_frames = get_frames(file, nof)
        videos = videos + dummy_vid
        del dummy_vid
        total_frames += num_frames
        print("Total Frames: ", total_frames, ", frames is file: ", num_frames)
...
def deconv3d(input_, output_shape, k_h=5, k_w=5, k_z=5, d_h=2, d_w=2, d_z=2,
    ...
    with tf.variable_scope(name):
        ...
        try:
          deconv = tf.nn.conv2d_transpose(input_, w, output_shape=output_shape, strides=[1, d_h, d_w, d_z, 1])

        # Support for verisons of TensorFlow before 0.7.0
        except AttributeError:
          deconv = tf.nn.deconv2d(input_, w, output_shape=output_shape, strides=[1, d_h, d_w, d_z, 1])
